...
object Lenses {
  ...
  val l2 = Lens[String, (String, Int)] ((_, 0)) (t1 => _ => t1._1)

  // page 7 in Foster et al.

  val l3 = Lens[(String,Int), String] (_._1) (s1 => {
  ...
  // the answer below:
  //
  // Since we do not need to modify anything in the test, we do not have to do the annoying copying that was the whole
  // reason for using lenses.
  ...
  // compose (append) a traversal and ^|-> is used to append a lense.
   ...
   val itu4 = (_students ^|->> filterIndex((str: String) => str(0) == 'A') ^|-> _country).modify(_.toUpperCase) (itu)
...
//   println (itu4)
  ...
  def setIth[A] (n: Integer) :Optional[List[A],A] =
    Optional[List[A], A] (_.lift(n)) (a => as => if (n < as.size) as.updated(n,a) else as)
  ...
  def setIth1[A] (n: Integer, default: A) :Lens[List[A],A] =
    Lens[List[A], A] (as =>  if (n < as.size) as(n) else default) (a => as => {
      if (n < as.size) as.updated(n,a)
      else {
        val gapSize = n - as.size
        val filler = List.fill(gapSize)(default)
        as:::filler:::List(a)
      }
    })
  ...
  // list list0
   ...
   val list1 = setIth[Int](2).modify(_+1)(list0)
   println (list0)
   println (list1)
