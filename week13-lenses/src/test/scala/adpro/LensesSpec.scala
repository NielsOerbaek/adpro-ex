...
class LensesSpec extends FlatSpec with Checkers {
  ...
  // concrete lenses.
   ...
   "l3" should behave like veryWellBehavedTotalLense (l3) // will fail PutPut
  ...
  def PartialPutGet[C,A] (l: Optional[C,A])
                         (implicit aA: Arbitrary[A], aC: Arbitrary[C]) :Prop =
    forAll { (a: A, c: C) => l.getOption(l.set(a)(c)) match {
      case Some(a2) if a2 == a => true
      case None => true
      case _ => false
    } }
  ...
  def PartialGetPut[C,A] (l: Optional[C,A])
                         (implicit aC: Arbitrary[C]) :Prop =
    forAll { (c: C) => l.getOption(c) match {
      case None => true
      case Some(a) => l.set(a)(c) == c
    }}
...
// that we can quickly test whether our implemented lenses fullfil the laws using
