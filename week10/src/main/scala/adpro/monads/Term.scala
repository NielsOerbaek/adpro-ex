...
object OutputEvaluator {
  ...
  def eval (term :Term) :M[Int] = term match {
  	...
  	case Div (t,u) => {
  		val evalT = eval(t)
  		val (x,a) = (evalT.o, evalT.a)
  		val evalU = eval(u)
  		val (y,b) = (evalU.o, evalU.a)
...
object ExceptionEvaluatorWithMonads {
  ...
  // Answer: Because we explicitly wrap the answer in an M (Return or Raise), 
  // we use flatMap-flatmap instead of flatMap-map.
  // We could also have put this check inside Unit as in the paper, and kept
  // the eval function the same.
}
...
object StateEvaluatorWithMonads {
  ...
  def eval (term :Term) :M[State] = term match {
    case Cons (a) => M.unit (a)
    case Div (t,u) => eval (t) flatMap (a => eval (u) flatMap (b => tick map (c => a/b)))
  }

  //The zero could be whatever, since we just ignore it.
  def tick : M[State] = M[State] { x => (0, x+1) }
...
object OutputEvaluatorWithMonads {
  ...
  // TODO: implement eval
  def eval (term :Term) :M[Int] = term match {
  	case Cons (a) => out(line(term)(a)) map (notADamnThing => a)
  	case Div (t,u) => eval (t) flatMap (a => {
  		eval (u) flatMap (b => {
  			out(line(term)(a/b)) map (notADamnThing => a/b)
  			})
  		})

  }

  def out(x: Output) = M[Unit] (x, ())
