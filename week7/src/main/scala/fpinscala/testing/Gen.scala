...
case class Gen[A] (sample :State[RNG,A]) {
  ...
  def listOfN (n :Int) :Gen[List[A]] = {
    val listOfSteps = List.fill(n)(sample)
    Gen(State.sequence(listOfSteps))
  }
...
object Gen {
  ...
  def anyNonNegativeInteger: Gen[Int] = 
    Gen(anyInteger.sample.map(i => if(i < 0) -i-1 else i))

  def choose (start :Int, stopExclusive :Int) :Gen[Int] = 
    Gen(anyNonNegativeInteger.sample.map(_ % (stopExclusive - start) + start))
  ...
  def boolean :Gen[Boolean] = 
    Gen(anyInteger.sample.map(_%2==0))
  ...
  def double :Gen[Double] = 
    Gen(anyNonNegativeInteger.sample.map(_.toDouble/((Int.MaxValue).toDouble + 1.0)))
