...
object data {
  ...
  trait Reduce[F[_]] {
    ...
    def toList[A] (fa: F[A]) :List[A] = reduceR[A, List[A]] (_::_) (fa, List())
  ...
  object ConsL {
    def unapply[A] (t: FingerTree[A]) :Option[(A,FingerTree[A])] = t match {
      ...
      case Deep(prHead::prTail,m,sf) => Some((prHead, FingerTree.deepL(prTail, m, sf)))
      case _ => None
    }
  }
  ...
  object FingerTree extends Reduce[FingerTree] { // uncomment once the interface is implemented
    ...
    def reduceR[A,Z] (opr: (A,Z) => Z) (t: FingerTree[A], z: Z) :Z = t match {
      ...
      case Deep(pr, m, sf) => Digit.reduceR (opr)(pr, reduceR (Node.reduceR (opr)) (m, Digit.reduceR (opr) (sf, z)))
    }
    ...
    def addL[A] (a: A, fa: FingerTree[A]) : FingerTree[A] = fa match {
      case Empty() => Single(a)
      case Single(b) => Deep(Digit(a), Empty(), Digit(b))
      case Deep(Digit(b,c,d,e), m, sf) => {
        val newM = addL[Node[A]] (Node3(c,d,e), m)
        Deep(Digit(a, b), newM, sf)
      }
