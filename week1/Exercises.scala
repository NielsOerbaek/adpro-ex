// Advanced Programming, Exercises by A. Wąsowski, IT University of Copenhagen
//
// AUTHOR1: Niels Ørbæk Christensen, niec@itu.dk
// AUTHOR2:
//
// Write ITU email addresses of both group members that contributed to
// the solution of the exercise (in lexicographic order).
//
// You should work with the file by following the associated exercise sheet
// (available in PDF from the course website).
//
// The file is meant to be compiled as follows:
//
// scalac Exercises.scala
//
// or
//
// fsc Exercises.scala
//
// To run the compiled file do "scala Exercises"
//
// To load the file int the REPL start the 'scala' interpreter and issue the
// command ':load Exercises.scala'. Now you can interactively experiment with
// your code.
//
// Continue solving exercises in the order presented in the PDF file. Large
// parts of the file are commented in order to make sure that the exercise
// compiles.  Uncomment and complete fragments as you proceed.  The file shall
// always compile and run after you are done with each exercise (if you do them
// in order).  Please compile and test frequently.

// The extension of App allows writing statements at class top level (the so
// called default constructor). For App objects they will be executed as if they
// were placed in the main method in Java.

object Exercises extends App {

  def power (x: Double, n: Int): Double = n match {
    case (0) => 1
    case n if (n < 0) => 1.0/(power (x, -n)) //Not tail position
    case n if (n%2 == 1) => x * power(x, n-1) //Not tail position
    case n if (n%2 == 0) => {
      val y = power(x, n/2) //Not tail position
      y*y
    }
  }

  // Exercise 3

  def fib(n: Int) : Int = {
    @annotation.tailrec
    def go(i: Int, current: Int, last: Int) : Int = {
      if(1 < i) {
        val next = current + last
        go(i-1, next, current)
      }
      else last
    }
    go(n,1,0)
  }

  // some tests:
  assert (fib (1) == 0)
  assert (fib (2) == 1)
  assert (fib (3) == 1)
  assert (fib (4) == 2)
  assert (fib (5) == 3)
  assert (fib (6) == 5)
  assert (fib (7) == 8)
  assert (fib (8) == 13)

  // Exercise 4

  // A simple object describing a cost line; implemented imperatively, Java
  // style (this way until we learn more Scala)
  class Expense {

    // A constructor definition
    def this (tag :String, price :Int) = {
      this()
      this.tag = tag
      this.price = price
    }

    var tag   :String = "" // a tag line in the accounting system
    var price :Int    = 0 // the price is in cents
  }

  // computes the total of expenses in cents

  def total (expenses: Array[Expense]) :Int = {
    @annotation.tailrec
    def go(index: Int, sum: Int): Int = {
      if(index >= 0) go(index-1, sum+expenses(index).price)
      else sum
    }
    go(expenses.length-1, 0)
  }

  val testcase1 = Array[Expense](
    new Expense("Coffee", 450),
    new Expense("Cake", 350) )

  assert (total (testcase1) == 800)

  val testcase2 = Array[Expense](
    new Expense("Coffee", 600),
    new Expense("Free Biscuit", 0),
    new Expense("Cheese Cake", 800) )

  assert (total (testcase2) == 1400)

  // Exercise 5

  def isSorted[A] (as: Array[A], ordered: (A,A) =>  Boolean) :Boolean = {
    @annotation.tailrec
    def go(index: Int): Boolean = {
      if(index < as.length-1){
        if(ordered(as(index),as(index+1)))
          go(index+1)
        else false
      }
      else true
    }
    go(0)
  }

  // some tests (uncomment)

  assert ( isSorted (Array(1,2,3,4,5,6), (a: Int, b: Int)=> a <= b))
  assert (!isSorted (Array(6,2,3,4,5,6), (a: Int, b: Int)=> a <= b))
  assert (!isSorted (Array(1,2,3,4,5,1), (a: Int, b: Int)=> a <= b))

  // add two tests with another type, for example an Array[String]

  assert ( isSorted (Array("abe","banan","coconut","diva"), 
    (a: String, b: String)=> a.compareTo(b) <= 0))
  assert ( isSorted (Array("abe","abe","coconut","diva"), 
    (a: String, b: String)=> a.compareTo(b) <= 0))
  assert ( !isSorted (Array("banan","abe","coconut","diva"), 
    (a: String, b: String)=> a.compareTo(b) <= 0))


  // Exercise 6

  def curry[A,B,C] (f: (A,B)=>C) : A => (B => C) = {
    (a: A) => ((b: B) => f(a,b))
  }

  // test if it type checks by currying power automatically:

  val power_curried: Double => Int => Double = 
    curry(power)
  val power2: Int => Double = power_curried(2)
  assert (power2(0) == 1.0)
  assert (power2(1) == 2.0)
  assert (power2(2) == 4.0)
  assert (power2(3) == 8.0)
  assert (power2(4) == 16.0)
  
  // Exercise 7

  def uncurry[A,B,C] (f: A => B => C) : (A,B) => C = {
    (a: A, b: B) => f(a)(b)
  }

  val power_uncurried: (Double,Int) => Double = 
    uncurry(power_curried)
  
  assert (power_uncurried(2,8) == 256.0)

  //I don't know how to do the thing with isSorted. 

 // Exercise 8

  def compose[A,B,C] (f: B => C, g: A => B) : A => C = {
    (a: A) => f(g(a))
  }

  val plus2Divide3: Int => Double = 
    compose((b: Int) => b/3.0, (a: Int) => a+2)

  assert(plus2Divide3(4) == 2.0)
  assert(plus2Divide3(7) == 3.0)
  assert(plus2Divide3(10) == 4.0)
  */

}
